...
public class MyScript : IScriptRunner
{
    ...
    private bool JumpByXLetters(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private bool JumpByLetterValues(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private bool JumpByPrimeNumbers(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private bool JumpByFibonacciNumbers(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private bool JumpByPiDigits(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private bool JumpByEulerDigits(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private bool JumpByGoldenRatioDigits(Client client, string extra)
    {
        if (client == null) return false;
        if (client.Selection == null) return false;
        List<Verse> verses = client.Selection.Verses;
    ...
    private char[] DoJumpByXLetters(Client client, string verses_text, string extra)
    {
    ...
    private char[] DoJumpByLetterValues(Client client, string verses_text, string extra)
    {
        int count = verses_text.Length;
        char[] result = new char[count];
        //text = verses_text.Replace("\r\n", "_"); // will miss 15 \n without \r before them
    ...
    private char[] DoJumpByPrimeNumbers(Client client, string verses_text, string extra)
    {
        //text = verses_text.Replace("\r\n", "_"); // will miss 15 \n without \r before them
        verses_text = verses_text.Replace("\r", "");
        verses_text = verses_text.Replace("\n", "_");
    ...
    private char[] DoJumpByFibonacciNumbers(Client client, string verses_text, string extra)
    {
        //text = text.Replace("\r\n", "_"); // will miss 15 \n without \r before them
        verses_text = verses_text.Replace("\r", "");
        verses_text = verses_text.Replace("\n", "_");
    ...
    private char[] DoJumpByConstantNumberDigits(Client client, string verses_text, string extra)
    {
        //text = text.Replace("\r\n", "_"); // will miss 15 \n without \r before them
        verses_text = verses_text.Replace("\r", "");
        verses_text = verses_text.Replace("\n", "_");
        ...
        for (int i = 0; i < count; /* i++ */) // advance inside loop
        {
            ...
            switch (extra)
            {
                case "PI":
                    {
                        j_count = Numbers.PiDigits[d++];
                    }
                    break;
                case "E":
                    {
                        j_count = Numbers.EDigits[d++];
                    }
                    break;
                case "PHI":
                    {
                        j_count = Numbers.PhiDigits[d++];
                    }
                    break;
            }
    ...
    private bool WordValuesSound(Client client, string extra)
    {
        ...
        try
        {
            ...
            foreach (Verse verse in verses)
            {
                foreach (Word word in verse.Words)
                {
                    values.Add((int)client.CalculateValue(word));
                }
            }
        ...
        try
        {
            ...
            foreach (Verse verse in verses)
            {
                for (int w = 0; w < verse.Words.Count; w++)
                {
                    ...
                    {
                        values.Add((int)client.CalculateValue(verse, w, l));
                    }
                }
            }
    ...
    private bool ChapterValueDiffsSound(Client client, string extra)
    {
        ...
        try
        {
            ...
            List<int> value_diffs = new List<int>();
            for (int i = 0; i < values.Count - 1; i++)
            {
                int value_diff = values[i + 1] - values[i];
    ...
    private void DoSaveAndPlayWAVFile(string filename, List<int> values, string extra)
    {
    ...
    private bool ChapterValueDigitSums(Client client, string extra)
    {
        ...
        try
        {
            ...
            foreach (Chapter chapter in chapters)
            {
                long value = client.CalculateValue(chapter.Verses);
                int digit_sum = Numbers.DigitSum(value);
    ...
    private string DoChapterStatistics(Client client, List<Chapter> chapters)
    {
        ...
        foreach (Chapter chapter in chapters)
        {
            ...
            long chapter_value = 0;
            foreach (Verse verse in chapter.Verses)
            {
                chapter_word_count += verse.Words.Count;
                chapter_letter_count += verse.LetterCount;
                chapter_value += verse.Value;
            }
            chapter_unique_letter_count = chapter.UniqueLetters.Count;
            str.Append(chapter_word_count.ToString() + Globals.DELIMITER);
            str.Append(chapter_letter_count.ToString() + Globals.DELIMITER);
            str.Append(chapter_unique_letter_count.ToString() + Globals.DELIMITER);
