...
public class Server : IPublisher
{
        ...
        if (s_numerology_system != null)
        {
            ...
            switch (CalculationMode)
            {
                ...
                case CalculationMode.SumOfLetterValueDigitSums:
                    {
                        foreach (string word_text in word_texts)
                        {
                            foreach (char c in word_text)
                            {
                                result += Numbers.DigitSum(s_numerology_system.CalculateValue(c));
                            }
                        }
                    }
                    break;
                case CalculationMode.SumOfLetterValueDigitalRoots:
                    {
                        foreach (string word_text in word_texts)
                        {
                            foreach (char c in word_text)
                            {
                                result += Numbers.DigitalRoot(s_numerology_system.CalculateValue(c));
                            }
                        }
                    }
                    break;
                ...
                case CalculationMode.SumOfVerseValueDigitSums:
                    {
                        foreach (string verse_text in line_texts)
                        {
                            long verse_value = 0L;
                ...
                case CalculationMode.SumOfConcatenatedLetterValueDigitSums:
                    {
                        foreach (string word_text in word_texts)
                        {
                            ...
                            {
                                word_value += Numbers.DigitSum(s_numerology_system.CalculateValue(word_text[i])) * (long)Math.Pow(10, i);
                            }
                            result += word_value;
                        }
                    }
                ...
                case CalculationMode.SumOfLetterValueDigitSums:
                    {
                        foreach (char c in text)
                        {
                            ...
                            {
                                Log.Append(c.ToString());
                                long value = Numbers.DigitSum(s_numerology_system.CalculateValue(c));
                                result += value; ValueSum += value;
                                Log.AppendLine("\t" + value);
                            }
                        }
                ...
                case CalculationMode.SumOfLetterValueDigitalRoots:
                    {
                        foreach (char c in text)
                        {
                            ...
                            {
                                Log.Append(c.ToString());
                                long value = Numbers.DigitalRoot(s_numerology_system.CalculateValue(c));
                                result += value; ValueSum += value;
                                Log.AppendLine("\t" + value);
                            }
                        }
                ...
                case CalculationMode.SumOfConcatenatedLetterValueDigitSums:
                    {
                        foreach (string word_text in word_texts)
                        {
                            ...
                            for (int i = 0; i < word_text.Length; i++)
                            {
                                ...
                                long value = Numbers.DigitSum(s_numerology_system.CalculateValue(word_text[i])) * (long)Math.Pow(10, i);
                                word_value += value; ValueSum += value;
                                Log.AppendLine("\t" + value);
                            }
                            result += word_value;
                    ...
                    case CalculationMode.SumOfLetterValueDigitSums:
                        {
                            foreach (Letter letter in word.Letters)
                            {
                                result += Numbers.DigitSum(CalculateValue(letter));
                            }
                        }
                        break;
                    case CalculationMode.SumOfLetterValueDigitalRoots:
                        {
                            foreach (Letter letter in word.Letters)
                            {
                                result += Numbers.DigitalRoot(CalculateValue(letter));
                            }
                        }
                        break;
                    case CalculationMode.SumOfWordValueDigitSums:
                    ...
                    case CalculationMode.SumOfConcatenatedLetterValueDigitSums:
                        {
                            ...
                            for (int i = 0; i < word.Letters.Count; i++)
                            {
                                ...
                                value = Numbers.DigitSum(CalculateValue(word.Letters[i])) * (long)Math.Pow(10, i);
                                word_value += value; ValueSum += value;
                                Log.AppendLine("\t" + value);
                            }
                            result = word_value;
        ...
        if (s_numerology_system != null)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (CalculationMode == CalculationMode.SumOfVerseValueDigitalRoots)
                    {
    ...
    public static long CalculateValueWithLogging(Letter letter)
    {
        ...
        if (s_numerology_system != null)
        {
            ...
            {
                long value = 0L;
                if (CalculationMode == Model.CalculationMode.SumOfLetterValueDigitSums)
                {
                    value = Numbers.DigitSum(s_numerology_system.CalculateValue(letter.Character));
                }
                else if (CalculationMode == Model.CalculationMode.SumOfLetterValueDigitalRoots)
                {
                    value = Numbers.DigitSum(s_numerology_system.CalculateValue(letter.Character));
                }
                else
                    ...
                    case CalculationMode.SumOfConcatenatedLetterValueDigitSums:
                        {
                            ...
                            {
                                Log.Append(word.Letters[i].ToString());
                                value = Numbers.DigitSum(CalculateValueWithLogging(word.Letters[i])) * (long)Math.Pow(10, i);
                                word_value += value; ValueSum += value;
                            }
                            result = word_value;
                        }
            ...
            else
            {
                ...
                foreach (Word word in verse.Words)
                {
                    ...
                    if ((word.Letters != null) && (word.Letters.Count > 0))
                    {
                        ...
                        {
                            //????? must use last part of result instead of computing from word.Text 
                            value = s_numerology_system.CalculateValue(word.Text);
                            Log.Append("\t" + "\t" + value);
                            // adjust value of word
                            result += AdjustValueWithLogging(word);
    ...
    public static int CalculateLetterFrequencySum(string text, string phrase, FrequencySearchType frequency_search_type, bool? include_diacritics)
    {
        ...
        if (s_numerology_system != null)
        {
            if (include_diacritics == false) text = text.SimplifyTo(s_numerology_system.TextMode);
            text = text.Replace("\r", "");
            text = text.Replace("\n", "");
            text = text.Replace("\t", "");
            text = text.Replace("_", "");
            ...
            if (!String.IsNullOrEmpty(text))
            {
                if (include_diacritics == false) phrase = phrase.SimplifyTo(s_numerology_system.TextMode);
    ...
    public static bool IsMatchingLetters(string text_mode, string text, string phrase, FrequencyMatchingType frequency_matching_type, FrequencySearchType frequency_search_type, bool? include_diacritics)
    {
        ...
        else if (include_diacritics == false) { text = text.SimplifyTo(text_mode); }
        ...
        else if (include_diacritics == false) { phrase = phrase.SimplifyTo(text_mode); }
